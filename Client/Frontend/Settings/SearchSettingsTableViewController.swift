...
class SearchSettingsTableViewController: ThemedTableViewController, FeatureFlaggable {
    ...
    private let ItemSuggestionFirefox = 0
    private let ItemSuggestionSponsors = 1
        ...
        case .fxSuggestSettings:
            ...
            case ItemSuggestionFirefox:
                let setting = BoolSetting(
                    ...
                    titleText: String.localizedStringWithFormat(.Settings.Search.Suggest.SuggestionsFromFirefoxTitle,
                                                                AppName.shortName.rawValue),
                    statusText: String.localizedStringWithFormat(.Settings.Search.Suggest.SuggestionsfromFirefoxDescription,
                                                                 AppName.shortName.rawValue)
                )
            ...
            case ItemSuggestionSponsors:
                let setting = BoolSetting(
                    ...
                    statusText: String.localizedStringWithFormat(.Settings.Search.Suggest.SuggestionsFromSponsorsDescription,
                                                                 AppName.shortName.rawValue)
                )
                setting.onConfigureCell(cell, theme: themeManager.currentTheme)
    ...
    override func numberOfSections(in tableView: UITableView) -> Int {
        ...
        if featureFlags.isFeatureEnabled(.firefoxSuggestFeature, checking: .buildAndUser) {
            sectionsToDisplay.append(.fxSuggestSettings)
        }
    ...
    override func tableView(_ tableView: UITableView, editingStyleForRowAt indexPath: IndexPath) -> UITableViewCell.EditingStyle {
        let section = Section(rawValue: sectionsToDisplay[indexPath.section].rawValue) ?? .defaultEngine
        switch section {
        case .defaultEngine, .privateSession, .fxSuggestSettings:
