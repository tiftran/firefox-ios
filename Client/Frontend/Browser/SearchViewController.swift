...
class SearchViewController: SiteTableViewController,
    ...
    func searchRemoteTabs(for searchString: String) {
        ...
        self.filteredRemoteClientTabs = currentTabs.filter { value in
            ...
            if containsQueryParameters(url: tab.URL) {
                return false
            }
    ...
    private func filterData(data: Cursor<Site>) -> [Site?] {
        return data.filter { item in
            ...
            return !containsQueryParameters(url: url)
        }
    }

    func containsQueryParameters(url: URL) -> Bool {
        guard let components = URLComponents(url: url, resolvingAgainstBaseURL: false),
              let searchParams = components.queryItems,
              let paramMatchString = profile.prefs.stringForKey(PrefsKeys.FirefoxSuggestHideWithSearchParam)        else { return false }

        for searchParam in searchParams {
            let searchParamString = "\(searchParam.name)=\(searchParam.value ?? "" )"
            if searchParamString == paramMatchString {
                return true
            }
        }
        return false
